@echo off
:setup
REM Scoped because this shared helper is CALLed and must not leak state.
setlocal DisableDelayedExpansion
set "app.version=0.1.0"
set "app.name=MVS_Query"
set "app.rc=0"
set "app.self=%~f0"
for %%I in ("%~dp0..") do set "mvsq_project_root=%%~fI"
:main
set "mvsq_mode=%~1"
set "mvsq_fields=%~2"
set "mvsq_dump=%~3"
...
if not defined mvsq_caller set "mvsq_caller=MVS_Query.bat"
...
$ProjectRoot = [string]$env:mvsq_project_root
...
function Show-Usage {
    Write-Line ('MVS Explorer Toolkit ' + $env:mvsq_version)
    ...
    Write-Line 'MVS_DUMPS_ROOT (if defined), the toolkit directory, and common'
    Write-Line 'mvs_dumps_archive locations adjacent to the toolkit/current directory.'
}
...
$DumpFolder = Resolve-DumpFolder $Dump $ProjectRoot
...
foreach ($line in Get-Content -LiteralPath $idsPath -Encoding UTF8) {
    ...
    $note = ''
    if ($notesByTitle.ContainsKey($title)) { $note = (($notesByTitle[$title] | ForEach-Object { [string]$_ }) -join ' || ') }
