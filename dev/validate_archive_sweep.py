...
def main():
    ...
    test_all=check_batch(ROOT/"test"/"test_all.bat",("elapsed_ms","Diagnostics.Stopwatch","all-results.tsv","[TEST ","Project: MVS Explorer Toolkit","mvst_project_version=0.21.2"))
    if "expected_rc`tactual_rc`telapsed_ms" not in test_all:
        fail("test result TSV does not include elapsed_ms")
    check_batch(ROOT/"test"/"test_everything.bat",("[SUITE TEST ","Project version:","0.21.2","--start-workers","--max-workers"))
