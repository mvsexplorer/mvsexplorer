#!/usr/bin/env python3
"""Generate standalone two-dump comparison tools.
...
def main():
    ...
    template = read(DEV / "templates/compare.bat.tpl")
    batch_common = read(DEV / "library/batch-common.inc.bat")
    powershell = read(DEV / "library/compare-query.inc.ps1")
    ...
    for tool in spec["tools"]:
        ...
        values = {
            ...
            "ID_MODE": tool.get("id_mode", ""),
            "BATCH_COMMON": batch_common,
            "COMPARE_POWERSHELL": powershell,
        }
        for key, value in values.items():
            text = text.replace("@@" + key + "@@", value)
        if "@@" in text:
            raise ValueError("Unresolved comparison template marker: " + tool["name"])
